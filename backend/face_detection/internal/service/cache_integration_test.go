...
func TestFetchProfileFaceEmbForIDWithCache(t *testing.T) {
	t.Run("cache hit does not call db and increments hit counter", func(t *testing.T) {
		...
		service.FetchProfileFaceEmbForIDWithCache(ctx, 10, testCacheContainer.Client, testLogger, fetchFn)

		hitsBefore := getCounterValue(t, observability.CacheHitsTotal, "fetch_profile_face_emb_for_id")

		result, err := service.FetchProfileFaceEmbForIDWithCache(ctx, 10, testCacheContainer.Client, testLogger, fetchFn)
		require.NoError(t, err)
	...
	t.Run("cache miss calls db and increments miss counter", func(t *testing.T) {
		...
		result, err := service.FetchProfileFaceEmbForIDWithCache(context.Background(), 20, testCacheContainer.Client, testLogger, fetchFn)
		require.NoError(t, err)
		assert.Equal(t, profiles, result)
	...
	t.Run("db error is returned and increments error counter", func(t *testing.T) {
		...
		result, err := service.FetchProfileFaceEmbForIDWithCache(context.Background(), 30, testCacheContainer.Client, testLogger, fetchFn)
	...
	t.Run("different profile IDs are cached under separate keys", func(t *testing.T) {
		...
		ctx := context.Background()
		service.FetchProfileFaceEmbForIDWithCache(ctx, 1, testCacheContainer.Client, testLogger, makeFetch(1, profiles1))
		service.FetchProfileFaceEmbForIDWithCache(ctx, 2, testCacheContainer.Client, testLogger, makeFetch(2, profiles2))

		r1, _ := service.FetchProfileFaceEmbForIDWithCache(ctx, 1, testCacheContainer.Client, testLogger, makeFetch(1, profiles1))
		r2, _ := service.FetchProfileFaceEmbForIDWithCache(ctx, 2, testCacheContainer.Client, testLogger, makeFetch(2, profiles2))
...
func TestFetchAllVisitorDataWithCache(t *testing.T) {
	t.Run("cache hit does not call db and increments hit counter", func(t *testing.T) {
		...
		ctx := context.Background()
		service.FetchAllVisitorDataWithCache(ctx, 10, testCacheContainer.Client, testLogger, fetchFn)

		hitsBefore := getCounterValue(t, observability.CacheHitsTotal, "fetch_all_visitor_data")

		result, err := service.FetchAllVisitorDataWithCache(ctx, 10, testCacheContainer.Client, testLogger, fetchFn)
		require.NoError(t, err)
	...
	t.Run("cache miss calls db and increments miss counter", func(t *testing.T) {
		...
		result, err := service.FetchAllVisitorDataWithCache(context.Background(), 20, testCacheContainer.Client, testLogger, fetchFn)
		require.NoError(t, err)
		assert.Equal(t, visitors, result)
		assert.Equal(t, 1, dbCallCount)
	...
	t.Run("db error is returned", func(t *testing.T) {
		...
		result, err := service.FetchAllVisitorDataWithCache(context.Background(), 30, testCacheContainer.Client, testLogger, fetchFn)
		assert.ErrorIs(t, err, dbErr)
		assert.Nil(t, result)
	})
	...
	t.Run("different profile IDs are cached under separate keys", func(t *testing.T) {
		...
		ctx := context.Background()
		service.FetchAllVisitorDataWithCache(ctx, 1, testCacheContainer.Client, testLogger, makeFetch(1, v1))
		service.FetchAllVisitorDataWithCache(ctx, 2, testCacheContainer.Client, testLogger, makeFetch(2, v2))

		r1, _ := service.FetchAllVisitorDataWithCache(ctx, 1, testCacheContainer.Client, testLogger, makeFetch(1, v1))
		r2, _ := service.FetchAllVisitorDataWithCache(ctx, 2, testCacheContainer.Client, testLogger, makeFetch(2, v2))
...
func TestAppendToVisitorDataCache(t *testing.T) {
	t.Run("appends entry to existing cache so next fetch is a cache hit with updated data", func(t *testing.T) {
		...
		ctx := context.Background()
		service.FetchAllVisitorDataWithCache(ctx, 10, testCacheContainer.Client, testLogger, func() ([]service.VisitorFaces, error) {
			return initial, nil
		})

		service.AppendToVisitorDataCache(ctx, 10, testCacheContainer.Client, newEntry)

		dbCallCount := 0
		result, err := service.FetchAllVisitorDataWithCache(ctx, 10, testCacheContainer.Client, testLogger, func() ([]service.VisitorFaces, error) {
			dbCallCount++
			return initial, nil
		})
	...
	t.Run("is noop when cache key does not exist", func(t *testing.T) {
		...
		ctx := context.Background()
		service.AppendToVisitorDataCache(ctx, 999, testCacheContainer.Client, makeVisitorFaces(99, "Ghost"))

		dbCallCount := 0
		result, err := service.FetchAllVisitorDataWithCache(ctx, 999, testCacheContainer.Client, testLogger, func() ([]service.VisitorFaces, error) {
			dbCallCount++
	...
	t.Run("does not affect cache for a different profile ID", func(t *testing.T) {
		...
		ctx := context.Background()
		service.FetchAllVisitorDataWithCache(ctx, 1, testCacheContainer.Client, testLogger, func() ([]service.VisitorFaces, error) { return v1, nil })
		service.FetchAllVisitorDataWithCache(ctx, 2, testCacheContainer.Client, testLogger, func() ([]service.VisitorFaces, error) { return v2, nil })

		service.AppendToVisitorDataCache(ctx, 1, testCacheContainer.Client, makeVisitorFaces(99, "Extra"))

		dbCallCount := 0
		result2, err := service.FetchAllVisitorDataWithCache(ctx, 2, testCacheContainer.Client, testLogger, func() ([]service.VisitorFaces, error) {
			dbCallCount++
			return v2, nil
		})
