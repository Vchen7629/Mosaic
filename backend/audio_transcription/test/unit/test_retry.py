...
def _make_func(side_effects, max_retries=3, initial_delay=1.0, backoff_multiplier=2.0, max_delay=30.0):
    mock_fn = MagicMock(side_effect=side_effects)

    @retry_with_backoff(max_retries=max_retries, initial_delay=initial_delay, backoff_multiplier=backoff_multiplier, max_delay=max_delay)
    def func():
        return mock_fn()
...
def test_retryable_error_logs_warnings_then_raises_and_logs_error_after_max_retries():
    ...
    mock_logger.error.assert_called_once_with("failed after max retries", err="conn failed")
